...
/**
 * 文件管理UI类（包含控件：ListView、OptionMenu、ContextMenu、Button等） 依赖类
 * 1.EventHandler（用于处理点击事件）、 2.FileManager（处理文件操作，如复制、粘贴、添加、删除、压缩、解压等）
 * 
 * @author Joe Berria /zyc
 * 
 */
public final class FileListActivity extends ListActivity implements
		OnClickListener {
	...
	private TextView title, mPathLabel, mDetailLabel;
	private View backButton, actionbarView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		location = SharePreferenceUtil.getPreference(this, YCFTPApplication.ROOT_DIR_SP, YCFTPApplication.ROOT_DIR);
		if(StringUtil.isEmpty(location)){
			location = "/";
		}
		if (savedInstanceState != null) {
			mHandler = new FileOprHandler(FileListActivity.this, mFileMag,
					savedInstanceState.getString("location"));
		} else {
			mHandler = new FileOprHandler(FileListActivity.this, mFileMag,location);
		}
	...
	private void initViews() {
		...
		title = (TextView) actionbarView.findViewById(R.id.header_title);
		title.setText(R.string.files_dir);

		backButton = actionbarView.findViewById(R.id.backButton);
		backButton.setOnClickListener(this);
		if (intent.getAction().equals(Intent.ACTION_GET_CONTENT)) {
			fileListAdapter = mHandler.new FileListAdapter(false);
		} else {
		...
		int[] img_button_id = { R.id.home_button, R.id.back_button,
				R.id.info_button, R.id.multiselect_button, R.id.actionBtn };
		...
		for (int i = 0; i < img_button_id.length; i++) {
			...
			if (i == 4) {
				// 取路径时显示确定按钮
				bimg[i] = actionbarView.findViewById(img_button_id[i]);
				if (intent.getAction().equals(Intent.ACTION_GET_CONTENT)) {
					bimg[i].setVisibility(View.VISIBLE);
					bimg[i].setOnClickListener(this);
				}
			} else {
				bimg[i] = findViewById(img_button_id[i]);
		...
		if (intent.getAction().equals(Intent.ACTION_GET_CONTENT)) {
			bimg[3].setVisibility(View.GONE);
			mReturnIntent = true;
	...
	/**
	 * 为通过startActivityForReresult向此类请求文件数据的类返回文件
	 * 
	 * @param data
	 */
	private void returnIntentResults(File data) {
		...
		setResult(YCFTPApplication.ROOT_RESULT_CODE, resultIntent);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		// R.drawable.ic_action_new_folder_dark);
		// menu.add(0, MENU_SEARCH, 0,
		// "Search").setIcon(R.drawable.ic_action_search_dark);
		// menu.add(0, MENU_SETTING, 0,
		// "Settings").setIcon(R.drawable.ic_action_settings_dark);
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		for (int i = 0; i < menu.size(); i++) {
			...
			case R.id.newdir:
				menu.getItem(i).setIcon(R.drawable.ic_action_new_folder_dark);
				break;
			case R.id.search:
				menu.getItem(i).setIcon(R.drawable.ic_action_search_dark);
				break;
			case R.id.setting:
				menu.getItem(i).setIcon(R.drawable.ic_action_settings_dark);
				break;
			case R.id.root:
				menu.getItem(i).setIcon(R.drawable.home_dark);
				break;
			case R.id.select_all:
				menu.getItem(i).setIcon(R.drawable.ic_action_select_all_dark);
				break;
			case R.id.about:
				menu.getItem(i).setIcon(R.drawable.ic_action_about_dark);
				break;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.root:
			if (mHandler.isMultiSelected()) {
				...
				Toast.makeText(this, "Multi-select is now off",
						Toast.LENGTH_SHORT).show();
			}
		...
		case R.id.select_all:
			if (mHandler.isMultiSelected()) {
				...
				LinearLayout hidden_lay = (LinearLayout) findViewById(R.id.hidden_buttons);
				hidden_lay.setVisibility(LinearLayout.VISIBLE);
			}
			return true;
		case R.id.about:
			Intent info = new Intent(this, DirectoryInfo.class);
	...
	protected Dialog onCreateDialog(int id) {
		...
		case MENU_MKDIR:
			...
			icon.setImageResource(R.drawable.ic_action_new_folder_light);
		...
		case F_MENU_RENAME:
			...
			ImageView rename_icon = (ImageView) dialog
		...
		case R.id.backButton:
			...
			if (!current.equals("/")) {
				...
				if (mHandler.isMultiSelected()) {
					fileListAdapter.killMultiSelect(true);
					Toast.makeText(FileListActivity.this,
							"Multi-select is now off", Toast.LENGTH_SHORT)
							.show();
